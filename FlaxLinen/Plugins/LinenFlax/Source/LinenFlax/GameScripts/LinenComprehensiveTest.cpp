...
void LinenComprehensiveTest::SetupRelationshipsAndFactions()
{
    ...
    if (!relationshipSystem || !factionSystem)
        return;
...
void LinenComprehensiveTest::RunWorldSimulationScenario()
{
    ...
    if (factionSystem) {
        ...
        LOG(Info, "Player reputation with townspeople: {0} ({1})",
            townRep, String(factionSystem->GetReputationLevelName(factionSystem->GetReputationLevel("player", "townspeople")).c_str()));

        LOG(Info, "Player reputation with bandits: {0} ({1})",
            banditRep, String(factionSystem->GetReputationLevelName(factionSystem->GetReputationLevel("player", "bandits")).c_str()));
    }
