...
void WeatherSystem::PickNewWeather() {
    ...
    m_targetWeather.SetIsDangerous(
        newCondition == WeatherCondition::Thunderstorm ||
        newCondition == WeatherCondition::Blizzard ||
        (newCondition == WeatherCondition::Heatwave && intensity > 0.7f)
    );
...
float WeatherSystem::GetWeatherIntensity() const {
    if (m_isTransitioning) {
        ...
        float progress = m_targetWeather.GetTransitionProgress();
        return m_currentWeather.GetIntensity() * (1.0f - progress) + 
               m_targetWeather.GetIntensity() * progress;
    }
...
void WeatherSystem::ForceWeatherChange(WeatherCondition condition, float intensity, float transitionDuration) {
    ...
    m_targetWeather.SetIsDangerous(
        ...
        (condition == WeatherCondition::Heatwave && intensity > 0.7f)
    );
...
std::string WeatherSystem::WeatherConditionToString(WeatherCondition condition) const {
    switch (condition) {
        ...
        case WeatherCondition::Heatwave: return "Heatwave";
...
WeatherCondition WeatherSystem::StringToWeatherCondition(const std::string& str) const {
    if (str == "Clear") return WeatherCondition::Clear;
    if (str == "Cloudy") return WeatherCondition::Cloudy;
    if (str == "Overcast") return WeatherCondition::Overcast;
    if (str == "Foggy") return WeatherCondition::Foggy;
    if (str == "Rain") return WeatherCondition::Rain;
    if (str == "Thunderstorm") return WeatherCondition::Thunderstorm;
    if (str == "Snow") return WeatherCondition::Snow;
    if (str == "Blizzard") return WeatherCondition::Blizzard;
    if (str == "Heatwave") return WeatherCondition::Heatwave;
    if (str == "Windy") return WeatherCondition::Windy;
